...
void quiz()
{
	...
	abc[0] = {"Which of the following is not the characteristic of a class?","Generic","Friend","Inline","super",'c'};
...
void write_student()
{
	student st;
	ofstream outFile;
	outFile.open("lav.dat",ios::binary|ios::app);
	st.getdata();
...
void display_all()
{
	student st;
	ifstream inFile;
	inFile.open("lav.dat",ios::binary);
	if(!inFile)
	{
